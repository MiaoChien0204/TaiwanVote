...
2.  **No Calculated Fields \| 無計算欄位**: All numerical fields are
    preserved exactly as-is from the source Excel files. No derived
    calculations (such as vote percentages) are included in the raw
    data.

    所有數值欄位都完全保持 Excel
    檔案原貌。原始資料中不包含任何衍生計算（如得票率）。
