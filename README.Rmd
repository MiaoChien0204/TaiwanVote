...
-   `is_elected` / `is_recalled` (Winner status at specified administrative level 在指定行政層級的勝出狀態)
    -   ⚠️ **注意 Note**: 此欄位反映「在指定層級的勝負狀況」，非實際選舉結果 This field reflects "winning status at the specified level", not actual election outcomes
    -   **Determined by `adm_level` parameter**: Results calculated based on the specified administrative level
    -   **Election**: TRUE if candidate received most votes at the specified level
    -   **Recall**: TRUE if agree votes \> disagree votes at the specified level
